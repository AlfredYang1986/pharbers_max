    ...
    FileInput.prototype = {
        ...
        _initTemplateDefaults: function () {
            ...
            tModal = '<div class="modal-dialog modal-lg{rtl}" role="document">\n' +
                ...
                '  </div>\n' +
                '</div>\n';
            tProgress = '<div class="file_progress">\n' +
        ...
        _uploadSingle: function (i, allFiles) {
            ...
            updateUploadLog = function (i, previewId) {
                if (multiUploadMode || !uploadFailed) {
                    // fuck
                    /**
                     * 增加改参数判断，其原因是不符合项目的业务逻辑。
                     * 插件原上传逻辑：设置了最大上传文件数后，等文件上传结束后，还是可以再次填入文件进行上传（maxFileCount我理解是最大同时可上传文件，官方文档与api也未曾找到参数设置，也有可能我太菜）
                     * 更改后逻辑：设置了最大上传文件数后，通过true || false控制在文件上传件数后是否还可以进行添加文件上传
                     * 参数名：maxFileCountFlag
                     * 结果：true || false
                     * true：走插件原有逻辑
                     * false：走更改后的逻辑
                     */
                    if(self.maxFileCountFlag) {
