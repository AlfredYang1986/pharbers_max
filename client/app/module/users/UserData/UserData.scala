...
trait UserData {
	...
	def conditions(data: JsValue): DBObject = {
		...
		(data \ "condition" \ "showCompany").asOpt[String].map(x => builder += "profile.showCompany" -> x).getOrElse(Unit)
	...
	implicit val m2d: JsValue => DBObject = { js =>
		...
		val id = (js \ "user" \ "user_id").asOpt[String].map(x => x).getOrElse(Sercurity.md5Hash(email))

		val profile = DBObject(
			"email" -> email,
			"secret" -> password,
			"name" -> name,
			"phone" -> phone,
			"company" -> Sercurity.md5Hash(company),
			"showCompany" -> company,
