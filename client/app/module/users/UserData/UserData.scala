...
trait UserData {
	...
	implicit val m2d: JsValue => DBObject = { js =>
		...
		val password = (js \ "password").asOpt[String].map(x => x).getOrElse(throw new Exception(""))
		val secret = Sercurity.md5Hash(s"$email$password")
		val name = (js \ "name").asOpt[String].map(x => x).getOrElse(throw new Exception(""))
		val phone = (js \ "phone").asOpt[String].map(x => x).getOrElse(throw new Exception(""))
