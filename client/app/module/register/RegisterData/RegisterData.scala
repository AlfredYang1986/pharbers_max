...
trait RegisterData {
	...
	implicit val m2d: JsValue => DBObject = { js =>
		...
		val reg_content = Map("company" -> company, "linkman" -> linkman, "email" -> email, "phone" -> phone, "scope" -> Nil)
	...
	implicit val d2m: DBObject => Map[String, JsValue] = { obj =>
		...
		Map("email" -> toJson(reg_content.getAs[String]("email").map(x => x).getOrElse("")),
			"comapny" -> toJson(reg_content.getAs[String]("company").map(x => x).getOrElse("")),
