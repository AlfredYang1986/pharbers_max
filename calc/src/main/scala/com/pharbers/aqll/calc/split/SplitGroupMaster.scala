...
//  	val r : ArrayBuffer[List[(String, Double, Double)] => Option[(Int, Int, (Double, Double))]] = ArrayBuffer.empty
  	val r : ArrayBuffer[List[(String, Double, Double)] => Option[(Int, Int, (Double, Double), String, (String, Double, Double), String, String)]] = ArrayBuffer.empty
  	...
  	def receive = {
		...
		case SplitEventBus.average(avg) => {
			...
			val result = r.map (f => f(avg)).filterNot(_ == None).map(_.get).groupBy(x => (x._1, x._2, x._4)).map (x =>
								...
								(DateUtil.getDateLong(x._1._1, x._1._2), x._2.map (_._3._1).sum, x._2.map (_._3._2).sum, x._2.map(_._4).distinct, x._2.map{x => (x._5._1, x._5._2, x._5._3)} , x._2.map(_._6).distinct, x._2.map(_._7).head )))
