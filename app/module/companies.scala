...
abstract class msg_CompanyCommand extends CommonMessage("companies", UserModule)
...
object CompanyModule extends ModuleTrait with basemodel {
    ...
    def dispatchMsg(msg: MessageDefines)(pr: Option[Map[String, JsValue]])(implicit cm: CommonModules): (Option[Map[String, JsValue]], Option[JsValue]) = msg match {
        ...
        case msg_queryCompanyMulti(data) => queryMultiMacro(qcm, sr, MergeStepResult(data, pr), names, name)
        case _ => ???
    }

    override val name: String = "company"
    override val names: String = "companies"
    ...
    val dr : DBObject => Map[String, JsValue] = { obj =>
        Map(
            "company_id" -> toJson(obj.getAs[ObjectId]("_id").get.toString),
            "company_name" -> toJson(obj.getAs[String]("screen_name").get),
            "company_des" -> toJson(obj.getAs[String]("screen_photo").get)
        )
    }
    ...
    val d2m : JsValue => DBObject = { js =>
        ...
        builder += "company_name" -> (data \ "screen_name").asOpt[String].map (x => x).getOrElse("")
        builder += "company_des" -> (data \ "screen_photo").asOpt[String].map (x => x).getOrElse("")
