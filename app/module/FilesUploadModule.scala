...
object FilesUploadModuleMessage {
    sealed class msg_filesuploadBase extends CommonMessage("fileupload", FilesUploadModule)
...
object FilesUploadModule extends ModuleTrait {
  ...
  /**
    ...
    * @return
    */
  def scpCopyFiles_func(data : JsValue)(implicit cm: CommonModules): (Option[Map[String, JsValue]], Option[JsValue]) = {
    try {
  ...
  /**
    ...
    * @return
    */
  def removeFiles_func(data : JsValue)(implicit cm: CommonModules): (Option[Map[String, JsValue]], Option[JsValue]) = {
    try {
      val company = (data \ "company").asOpt[String].getOrElse(throw new Exception("error input"))
